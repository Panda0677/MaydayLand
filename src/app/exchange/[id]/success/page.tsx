...
type SwapSuccessPageProps = {
  params: { id: string };
  searchParams: { mine?: string; point?: string; time?: string; delivery?: string };
};

export default function SwapSuccessPage({ params, searchParams }: SwapSuccessPageProps) {
  ...
  const mine = searchParams.mine ?? formatExchangeItems(exchange.want);
  const point = searchParams.point ?? "体育场东门";
  const time = searchParams.time ?? "17:30–18:00";
  const delivery = searchParams.delivery === "local" || searchParams.delivery === "shipping" ? searchParams.delivery : "concert_meetup";
