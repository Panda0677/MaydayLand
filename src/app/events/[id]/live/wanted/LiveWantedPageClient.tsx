...
type LiveWantedPageClientProps = {
  eventId: string;
};

export default function LiveWantedPageClient({ eventId }: LiveWantedPageClientProps) {
      ...
      {success ? (
        <div className="fixed inset-0 z-50 mx-auto max-w-[390px]" role="dialog" aria-modal="true">
          ...
          <section className="absolute inset-x-4 top-1/2 -translate-y-1/2 rounded-2xl bg-white p-6 text-center shadow-soft">
            ...
            <PrimaryButton className="mt-5 w-full" href={`/events/${eventId}/live`}>
              返回现场模式
            </PrimaryButton>
          </section>
        </div>
